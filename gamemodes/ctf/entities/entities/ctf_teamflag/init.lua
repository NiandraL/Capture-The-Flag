...
function ENT:FlagStolen( pl )
	...
	PrintMessage( HUD_PRINTCENTER, "Flag stolen from " .. teamname .. " by " .. pl:Name() )
	
	for k, v in pairs(player.GetAll()) do
		v:ChatPrint(""..pl:Nick().." has picked up the "..teamname.." flag!")
	end	
	
	BroadcastLua([[sound.Play("radio/go.wav",LocalPlayer():GetPos())]])
end
...
function ENT:FlagReturned( pl )
	...
	if( pl and pl:IsValid() ) then
		local teamname = team.GetName( self.Team );
		PrintMessage( HUD_PRINTCENTER, "" .. teamname .. "'s flag returned by " .. pl:Name() );	
		...
		for k, v in pairs(player.GetAll()) do
			if v:Team() == teamname then
				v:ChatPrint("Your team's flag was successfully returned to base!")
			end	
		end	
	...
	if( ent and ent:IsValid() and ent:IsPlayer() ) then
		...
		elseif( ent:Team() == self.Team and !self.Stolen ) then
			...
			if( ent:Team() == TEAM_RED ) then
				enemy_flag = GetGlobalEntity( "flag_" .. tostring( TEAM_BLUECTF ) );
			...
			if( enemy_flag and enemy_flag:IsValid() ) then
				...
				if( stolen and carrier and carrier:IsValid() and carrier == ent ) then
					...
					PrintMessage( HUD_PRINTCENTER, ent:Name() .. " captured " ..teamname .. "'s flag" )
					ent:AddFrags(10)
...
function ENT:FlagDropped( pl )
	...
	pl:PS_HolsterItem("flag")
	
	for k, v in pairs(player.GetAll()) do
		v:ChatPrint(""..teamname.." flag has been dropped!")
	end	
