...
surface.CreateFont( "TrebBoldLarge", {
	font = "Trebuchet MS",
	size = 32,
	weight = 700,
	blursize = 0,
...
surface.CreateFont( "TrebBoldKillMsg", {
	font = "Trebuchet MS",
	size = 19,
	weight = 700,
	blursize = 0,
...
surface.CreateFont( "CS_HUDIcons", {
	font = "csd",
	size = 40,
	weight = 500,
	blursize = 0,
...
function GM:DrawHUDScores()
	...
	local tb_col = team.GetColor( TEAM_BLUECTF );
	surface.SetDrawColor( tb_col.r, tb_col.g, tb_col.b, 100 );
	surface.DrawRect( ( ScrW()/2 ) + ( 2 * sc ), 20 + ( 2.5 * sc ), 65 * sc, 45 * sc );	
	
	draw.SimpleText( tostring( team.GetScore( TEAM_BLUECTF ) ), "TrebBoldLarge", ( ScrW()/2 ) + ( 77 * sc ), 20 + ( 25 * sc ), color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER )
